...
def print_bar(img):
    ...
    plt.savefig(f"pics/reference/profile.png", dpi=70)


def convert_to_bin(img):
    if str(img.mode) != "1":
        img = mono(img)
    img.save("pics/reference/string.bmp")
    return img
...
def segmentation(img):
    ...
    for i in range(len(res)):
        ...
        new_im.save(f"pics/results/{str(i)}.bmp", mode="1")
    return res


def get_hist_profile(res):
    for i in range(len(res)):
        img = Image.open(f"pics/results/{str(i)}.bmp")
        ...
        plt.savefig(f"pics/hists/{i}.png", dpi=70)
        del fig
        del axs
...
img = Image.open("pics/reference/string.bmp")
img_bin = convert_to_bin(img)
# print_bar(img_bin)
arr = segmentation(img_bin)
print(arr)
get_hist_profile(arr)
segmentation(img_bin)
